...
import { Folder, Trash2, ArrowUpRight, Download, Upload, Calendar, Building2, CheckCircle2, AlertTriangle, Cloud, CloudLightning, Share2 } from "lucide-react";
...
interface ArchiveListProps {
  ...
  onExportHTML?: (archive: FilingArchive) => void;
  isLoggedIn?: boolean;
  currentUserId?: string;
}

export default function ArchiveList({ archives, onLoad, onDelete, onImport, onExportHTML, isLoggedIn, currentUserId }: ArchiveListProps) {
      ...
      {archives.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {archives.map((archive) => {
            ...
            const isOwner = !archive.createdBy || archive.createdBy === currentUserId;

            return (
              <div
                key={archive.id}
                className={`bg-white border hover:shadow-xs transition-all rounded-xl p-5 flex flex-col justify-between ${
                  isOwner ? "border-slate-200 hover:border-indigo-200" : "border-amber-200 hover:border-amber-300 bg-amber-50/5"
                }`}
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => onExportHTML?.(archive)}
                        title="他者へ転送可能なポータブルHTMLとして保存（メール添付用）"
                  ...
                  <div className="bg-slate-50 rounded-lg p-3 text-xs space-y-1.5">
                    ...
                    <div className="border-t border-slate-200/60 my-1.5 pt-1.5 space-y-1">
                      <p className="text-slate-600 flex justify-between items-center">
                        <span className="text-slate-400">作成者 (UID):</span>
                        <span className="font-mono text-[10px] text-slate-700 font-semibold">
                          {archive.createdBy ? (
                            archive.createdBy === currentUserId ? (
                              <span className="text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded font-sans font-bold">あなた (自分)</span>
                            ) : (
                              <span className="bg-slate-200/60 px-1.5 py-0.5 rounded text-slate-600 truncate max-w-[130px] inline-block align-middle" title={archive.createdBy}>
                                {archive.createdBy}
                              </span>
                            )
                          ) : (
                            <span className="text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded font-sans font-bold">ローカル保存</span>
                          )}
                        </span>
                      </p>
                      <p className="text-slate-600 flex justify-between">
                      ...
                      {archive.updatedAt && (
                        <p className="text-slate-600 flex justify-between">
                          ...
                          <span className="text-slate-700 font-medium">
                            {new Date(archive.updatedAt).toLocaleString("ja-JP")}
                          </span>
                        </p>
                      )}
                    </div>
                ...
                <div className="border-t border-slate-100 mt-5 pt-3 flex items-center justify-between text-xs text-slate-400">
                  <span className="flex items-center gap-1">
                    ...
                    <span>登録日: {new Date(archive.createdAt).toLocaleDateString("ja-JP")}</span>
                  </span>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      title="メール添付で他人に送れるインタラクティブHTMLを書き出します"
                    >
