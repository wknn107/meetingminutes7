...
import { UploadedFiles, CompanyInfo, GeneratedDocument, TaskType } from "./types";
...
import { generatePortableHtml } from "./lib/portableHtml";
import { generatePortableAppHtml } from "./lib/portableAppHtml";
      ...
      if (localSaved) {
        ...
        if (localArchives.length > 0 && confirm(`ローカルに保存されている${localArchives.length}件のデータをクラウド（アカウント）へバックアップ・同期しますか？`)) {
          ...
          for (const arc of localArchives) {
            const updatedArc: FilingArchive = {
              ...arc,
              createdBy: result.user.uid,
              updatedAt: arc.updatedAt || new Date().toISOString()
            };
            try {
              await setDoc(doc(db, path, arc.id), updatedArc);
            } catch (err) {
...
　　formData.append("file", uploadedFiles[0]); // 1つだけ送る
    ...
    const response = await fetch("/api/generate-docs", {
      method: "POST",
      body: formData, // ← Content-Type を自分で設定しない！
    });
...
　　　　}
  ...
  const handleSaveAllToArchive = async () => {
    const isUpdate = !!currentArchiveId;
    const archiveId = currentArchiveId || "archive-" + Date.now();
    
    // Find the existing archive to preserve its createdAt if updating
    const existingArchive = archives.find(a => a.id === archiveId);
    const createdAtTime = existingArchive ? existingArchive.createdAt : new Date().toISOString();
    ...
    if (user) {
      newArchive.createdBy = user.uid;
      setSyncing(true);
      const path = "archives";
      try {
        await setDoc(doc(db, path, archiveId), newArchive);
        if (isUpdate) {
          setArchives((prev) => prev.map((a) => a.id === archiveId ? newArchive : a));
          alert(`「${companyInfo.name || "未指定会社"}」の登記申請データを更新しました！`);
        } else {
        ...
        handleFirestoreError(err, isUpdate ? OperationType.UPDATE : OperationType.CREATE, `${path}/${archiveId}`);
      } finally {
        setSyncing(false);
      }
    } else {
      if (isUpdate) {
        const updated = archives.map((a) => a.id === archiveId ? newArchive : a);
        setArchives(updated);
        localStorage.setItem("commercial_registry_archives", JSON.stringify(updated));
        alert(`「${companyInfo.name || "未指定会社"}」の登記申請データをローカルで更新しました！`);
      } else {
  ...
  // ポータブルHTMLのエクスポート（現在のプロジェクト）
  const handleExportCurrentToPortableHTML = () => {
    if (documents.length === 0) {
      alert("エクスポートする登記申請書類がありません。まずはステップ1〜2で書類を生成してください。");
      return;
    }

    const archiveData: FilingArchive = {
    ...
    try {
      const htmlString = generatePortableHtml(archiveData);
      const blob = new Blob([htmlString], { type: "text/html;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `商業登記ポータブル_${companyInfo.name || "未指定会社"}.html`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error("Failed to export portable HTML:", err);
      alert("ポータブルHTMLのエクスポートに失敗しました。");
    }
  };

  // ポータブルHTMLのエクスポート（アーカイブ項目）
  const handleExportArchiveToPortableHTML = (archive: FilingArchive) => {
    try {
      const htmlString = generatePortableHtml(archive);
      const blob = new Blob([htmlString], { type: "text/html;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `商業登記ポータブル_${archive.companyInfo.name || "未指定会社"}.html`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error("Failed to export archive to portable HTML:", err);
      alert("アーカイブのポータブルHTMLエクスポートに失敗しました。");
    }
  };

  // アプリケーション自体の自己完結型ポータブルHTMLエクスポート（他の人が使える）
  const handleExportAppToPortableHTML = () => {
    try {
      const htmlString = generatePortableAppHtml();
      const blob = new Blob([htmlString], { type: "text/html;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "商業登記申請書類生成アシスタント_ポータブル版.html";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error("Failed to export portable app HTML:", err);
      alert("ポータブルアプリHTMLのエクスポートに失敗しました。");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans flex flex-col">
      {/* 印刷時にはナビゲーションを隠す */}
      <header className="bg-white text-slate-800 shadow-sm border-b border-slate-200 no-print">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight text-slate-900">商業登記申請書類生成アシスタント</h1>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <div className="flex items-center space-x-1.5 text-xs text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
              ...
              <span className="font-bold text-slate-700">Gemini 3.5 Flash 高速法務推論エンジン搭載</span>
            </div>

            {documents.length > 0 && (
              <button
                onClick={handleExportCurrentToPortableHTML}
                className="flex items-center gap-1.5 text-xs font-bold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 px-3.5 py-1.5 rounded-lg shadow-md hover:shadow-lg transition-all cursor-pointer border border-emerald-500/20"
                title="現在の会社情報・生成書類・タスクをすべて埋め込んだ単一のHTMLアプリとして保存し、メール添付などで他人に転送・共有できます。"
              >
                <Share2 className="w-3.5 h-3.5" />
                <span>作成データごと共有HTML</span>
              </button>
            )}

            <button
              onClick={handleExportAppToPortableHTML}
              className="flex items-center gap-1.5 text-xs font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 px-3.5 py-1.5 rounded-lg shadow-md hover:shadow-lg transition-all cursor-pointer border border-indigo-500/20"
              title="この書類生成アプリ全体を自己完結型のHTMLファイルとして保存します。他人に送信すると、その人も会社情報の入力から議事録作成・書類生成、タスク管理を行えるようになります。"
            >
              <Download className="w-3.5 h-3.5" />
              <span>アプリ自体をHTML保存</span>
            </button>

            {/* Firebase Auth Controls */}
            {authLoading ? (
              <div className="flex items-center text-xs text-slate-500 gap-1 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
              ...
              <div className="flex items-center gap-2">
                <div className="hidden md:flex flex-col items-end">
                  ...
                  <span className="text-[10px] text-slate-600 font-semibold max-w-[120px] truncate">{user.displayName || user.email}</span>
                </div>
                {user.photoURL && (
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[11px] font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer"
                  title="ログアウト"
                >
          ...
          {activeTab === "archive" && (
            ...
            >
              <ArchiveList
                ...
                onExportHTML={handleExportArchiveToPortableHTML}
                isLoggedIn={!!user}
